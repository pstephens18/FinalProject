...
public:
	...
	void enter_data(){
		...
		// Populate vectors with correct number of doctors / nurses
		...
		for (int i = 0; i <= 7*60*24; i++) { 
			...
			if (random(60, 0) < patientArivalRate && (doctorWaitlist->size() + nurseWaitlist->size() + nurses.size() + doctors.size()) != patients.size()) {
				...
				if ((patients.size() * 9) / 10 < (nurseWaitlist->size()+doctorWaitlist->size())) {
					...
					// Select someone randomly to get sick
					do {
	...
	void show_stats(){
		...
		for (int i = 0;i < patients.size();i++) {
			if (patients[i]->getPriority() != 0) {
				visits += patients[i]->getNumVisits();
		...
		for(int i = 0;i<100;i++){
			...
			// Find patient if they exist and output the number of times they visited and the severity of their illness
